...
Esta es una interfaz web diseñada para un avanzado sistema de síntesis de texto a voz (Text-to-Speech). La interfaz incluye un finetuning especializado que optimiza el modelo para generar voces en español con una fluidez y naturalidad excepcionales, adaptándose a los matices del idioma y las diferentes variantes regionales.

Este modelo de inteligencia artificial ha sido miniaturizado y optimizado exclusivamente para generar audio en español. Por lo tanto, no admite otros idiomas o variantes fuera de este ámbito lingüístico.

Para obtener los mejores resultados, asegúrate de que tu audio de referencia cumpla con los siguientes requisitos: debe estar en formato WAV o MP3, tener una duración de entre 11 y 14 segundos, e incluir un intervalo de entre 0,5 y 1 segundo de silencio tanto al inicio como al final. Es recomendable que el audio termine con el cierre completo de una frase para garantizar una reproducción clara y coherente.


** Nota importante: Si no proporcionas un texto de referencia, este será transcrito automáticamente utilizando Whisper, garantizando una interpretación precisa del audio. Para obtener resultados óptimos, utiliza clips de referencia cortos, preferiblemente de menos de 15 segundos de duración, y asegúrate de que el archivo de audio esté completamente cargado antes de iniciar el proceso de generación.

Información adicional:
Este sistema incorpora la librería num2words, que convierte automáticamente los números en palabras durante el proceso de síntesis. Esto mejora significativamente la fluidez y naturalidad de la voz generada, asegurando una pronunciación precisa y coherente en cualquier contexto. **
"""
    )
    gr.TabbedInterface(
